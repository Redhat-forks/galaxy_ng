...
def ansible_collectionversion_table(since, full_path, until, **kwargs):
    source_query = """COPY (
            ...
            LEFT OUTER JOIN "ansible_collectionversion_tags" ON (
                "ansible_collectionversion"."content_ptr_id" = "ansible_collectionversion_tags"."collectionversion_id"
                )
        ) TO STDOUT WITH CSV HEADER
    """
...
def galaxy_legacynamespace_table(since, full_path, until, **kwargs):
    source_query = """COPY (SELECT * from galaxy_legacynamespace) TO STDOUT WITH CSV HEADER
    """
...
def galaxy_legacyrole_table(since, full_path, until, **kwargs):
    source_query = """COPY (SELECT * from galaxy_legacyrole) TO STDOUT WITH CSV HEADER
    """
    return _simple_csv(full_path, "galaxy_legacyrole", source_query)


def _simple_csv(full_path, file_name, query, max_data_size=Package.MAX_DATA_SIZE):
